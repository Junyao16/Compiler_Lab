...
struct Type_ {
    enum { BASIC, ARRAY, STRUCTURE, FUNCTION, STRUCT_NAME } kind;
    union {
        // 基本类型
        enum { INT_t, FLOAT_t } basic;
        // 数组类型信息包括元素类型与数组大小构成
        struct { Type elem; int size; } array;
        // 结构体类型信息是一个链表
        struct { FieldList fieldlist; char structName[32]; } structure;
        // Function
        struct {Type returnType; FieldList paraList;} function;
    } u;
 };
