    ...
    if (curNode != NULL && strcmp(curNode->nodeName, "VarList") == 0)
    {
        if (curNode->firstChild != NULL && strcmp(curNode->firstChild->nodeName, "ParamDec") == 0)
        {
            ...
            if (curNode->firstChild->nextSibling != NULL && strcmp(curNode->firstChild->nextSibling->nodeName, "COMMA") == 0)
            {
                ...
                {
                    if (fieldlist != NULL)
                        fieldlist->tail = VarList(curNode->firstChild->nextSibling->nextSibling);
                    else
                        fieldlist = VarList(curNode->firstChild->nextSibling->nextSibling);
                }
            }
            return fieldlist;
        }
    ...
    if (curNode != NULL && strcmp(curNode->nodeName, "DefList") == 0)
    {
        if (curNode->firstChild != NULL && strcmp(curNode->firstChild->nodeName, "Def") == 0)
        {
            ...
            if (curNode->firstChild->nextSibling != NULL && strcmp(curNode->firstChild->nextSibling->nodeName, "DefList") == 0)
            {
                ...
                if (tmp == NULL)
                    fieldlist = DefList(curNode->firstChild->nextSibling, IsStruct);
                else
                {
                    while (tmp->tail != NULL)
                    {
    ...
    if (curNode != NULL && strcmp(curNode->nodeName, "DecList") == 0)
    {
        ...
        if (curNode->firstChild != NULL && strcmp(curNode->firstChild->nodeName, "Dec") == 0)
        {
            ...
            if (curNode->firstChild->nextSibling != NULL && strcmp(curNode->firstChild->nextSibling->nodeName, "COMMA") == 0)
            {
                ...
                {
                    if (fieldlist != NULL)
                        fieldlist->tail = DecList(curNode->firstChild->nextSibling->nextSibling, type, IsStruct);
                    else
                        fieldlist = DecList(curNode->firstChild->nextSibling->nextSibling, type, IsStruct);
                }
            }
        }
        return fieldlist;
                ...
                else if (curNode->firstChild->nextSibling != NULL && strcmp(curNode->firstChild->nextSibling->nodeName, "AND") == 0)
                {
                    if (curNode->firstChild->nextSibling->nextSibling != NULL && strcmp(curNode->firstChild->nextSibling->nextSibling->nodeName, "Exp") == 0)
                    {
                        ...
                        if (type1->kind == FUNCTION)
                            type1 = type1->u.function.returnType;
                        if (type2->kind == FUNCTION)
                            type2 = type2->u.function.returnType;
            ...
            else if (strcmp(curNode->firstChild->nodeName, "MINUS") == 0)
            {
                if (curNode->firstChild->nextSibling != NULL && strcmp(curNode->firstChild->nextSibling->nodeName, "Exp") == 0)
                {
                    ...
                    if (type->kind == FUNCTION)
                        type = type->u.function.returnType;
                    if (type->kind != BASIC)
                    {
...
int CompareType(Type type1, Type type2)
{
    ...
    if (type1->kind != type2->kind)
    {
        if (type2->kind == FUNCTION)
        {
            if (CompareType(type1, type2->u.function.returnType) == 1)
                return 1;
        }
        if (type1->kind == FUNCTION)
        {
            if (CompareType(type2, type1->u.function.returnType) == 1)
                return 1;
        }
        if (type1->kind == STRUCT_NAME && type2->kind == STRUCTURE)
        {
            if (strcmp(type1->u.structure.structName, type2->u.structure.structName) == 0)
                return 1;
            if (CompareFieldList(type1->u.structure.fieldlist, type2->u.structure.fieldlist) == 1)
                return 1;
        }
    ...
    {
        if (CompareType(type1->u.array.elem, type2->u.array.elem) == 1)
            return 1;
    }
    ...
    {
        if (CompareType(type1->u.function.returnType, type2->u.function.returnType) == 1)
            return 1;
    }
    return 0;
}
...
{
    if ((fieldlist1 == NULL || fieldlist2 == NULL) && fieldlist1 != fieldlist2)
        return 0;
    FieldList tmp1 = fieldlist1;
    FieldList tmp2 = fieldlist2;
    while (tmp1 != NULL && tmp2 != NULL)
    {
        if (CompareType(tmp1->type, tmp2->type) != 1)
            return 0;
        tmp1 = tmp1->tail;
        tmp2 = tmp2->tail;
    }
